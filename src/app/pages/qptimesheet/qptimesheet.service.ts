...
import { filters, sortingObj, SaveTimeSheet, returnTimesheet,TimesheetreturnBO } from './qptimesheet.model';
...
export class QPTimesheetService {
  ...
  saveTimesheet(value: any): Observable<any> {
    let url = "api/Timesheet/saveQpTimesheet"
  ...
  updateTimesheetdata(value: any): Observable<any> {
    let url = "api/Timesheet/UpdateQpTimesheet"
    return this.http.post<any>(url, value, { observe: 'response' }).pipe(
      map(data => data.body),
      catchError(this.errorService.handleError)
    )
  ...
  deleteTimesheetdata(value: any) {
    let url="api/Timesheet/DeleteQpTimesheet";   
    return this.http.post<any>(url, value, { observe: 'response' }).pipe(
      map(data => data.body),
      catchError(this.errorService.handleError)
    )
