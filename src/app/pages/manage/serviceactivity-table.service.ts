...
export class GetHTTPService2 extends Subject<DataStateChangeEventArgs>{
  ...
  getCareList(state?: GetMSListBo): Observable<DataStateChangeEventArgs> {
    return this.http.post<any>("api/MasterServiceActivity/GetMasterServiceActivityList?", state)
      .pipe(map(
        (response: any) =>
          <any>{
            result: response['msaList'],
